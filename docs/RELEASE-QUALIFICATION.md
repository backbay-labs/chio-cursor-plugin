...
This lane builds `chio-cursor` from one immutable source commit. A passing workflow
...
- Public repository identity: `backbay-labs/chio-cursor-plugin`.
- Workflow: `.github/workflows/release.yml`.
- Registry package: `chio-cursor`; GitHub environment: `npm`.
...
5. An npm maintainer must register this package's Trusted Publisher with GitHub
   owner `backbay-labs`, repository `chio-cursor-plugin`, workflow filename `release.yml`,
...
cosign verify-blob --certificate SHA256SUMS.pem --signature SHA256SUMS.sig \
  --certificate-identity 'https://github.com/backbay-labs/chio-cursor-plugin/.github/workflows/release.yml@refs/tags/v0.3.0' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com SHA256SUMS
slsa-verifier verify-artifact package.tgz \
  --provenance-path package.intoto.jsonl \
  --source-uri github.com/backbay-labs/chio-cursor-plugin --source-tag 'v0.3.0'
```
...
Cursor also emits `chio-cursor.vsix` using `npm run package -- --out ...`. The
VSIX is checksum-bound and included in the SLSA subjects and GitHub assets. This
workflow does not publish to VS Code Marketplace or require `VSCE_PAT`; the
CLI tarball is the protected-mode delivery and VSIX sideloading remains subject
to its own Cursor host acceptance.
...
and 33 unit tests passed with zero failed or skipped tests. Staged packing,
